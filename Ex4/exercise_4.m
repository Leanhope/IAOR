...
figure; imshow(test1B); %imshow(bmtest1B);
hold on
...
figure; imshow(test2B); %imshow(bmtest2B);
hold on
for k = 1:length(img2descriptors)
    if norm(Df_train - img2descriptors(k, :)') < 0.06 % we could not find the ampelmaennchen with 0.06 threshold
        boundary = valuesB2{k};
        plot(boundary(:,2),  boundary(:,1), 'r', 'LineWidth', 2); 
    endif
endfor
...
figure; imshow(test3B)%imshow(bmtest3B);
hold on
for k = 1:length(img3descriptors)
    if norm(Df_train - img3descriptors(k, :)') < 0.07
        y = k
        x = norm(Df_train - img3descriptors(k, :)')
        z = 2000
        boundary = valuesB3{k};
        plot(boundary(:,2),  boundary(:,1), 'r', 'LineWidth', 2); 
    endif
